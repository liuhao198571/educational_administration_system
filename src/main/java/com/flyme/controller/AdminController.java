...
import com.flyme.exception.CustomException;
import com.flyme.po.*;
import com.flyme.service.*;
...
public class AdminController {
    ...
    @Resource(name = "chartsServiceImpl")
    private ChartsService chartsService;

    private PagingVO pagingVO;
    private OneCharts oneCharts;
    private TwoCharts twoCharts = new TwoCharts();

    /*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<管理员首页>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
    @RequestMapping("/index")
    public String index(Model model) throws Exception {
        // 获取课程，学生，教师各个总数，并且设置到OneCharts对象中
        oneCharts = new OneCharts();
        int countCouse = courseService.getCountCouse();
        oneCharts.setCourseCount(countCouse);
        int countStudent = studentService.getCountStudent();
        oneCharts.setStudentCount(countStudent);
        int countTeacher = teacherService.getCountTeacher();
        oneCharts.setTeacherCount(countTeacher);


        int degreeA = chartsService.getDegreeA();
        int degreeB = chartsService.getDegreeB();
        int degreeC = chartsService.getDegreeC();


        model.addAttribute("degreeA", degreeA);
        model.addAttribute("degreeB", degreeB);
        model.addAttribute("degreeC", degreeC);
    ...
    // 返回课程，学生，教师 各个总数，返回json
    @ResponseBody
    @RequestMapping(value = "/indexjson", method = RequestMethod.GET, produces = "application/json;charset=utf-8")
    public String indexJosn() throws Exception {
        String result = JSON.toJSONString(oneCharts);
        return result;
    }

    // 返回 教师 本科，硕士，博士 各个总数，返回json
    @ResponseBody
    @RequestMapping(value = "/indexjson2", method = RequestMethod.GET, produces = "application/json;charset=utf-8")
    public String indexJosn2() throws Exception {
        int degreeA = chartsService.getDegreeA();
        int degreeB = chartsService.getDegreeB();
        int degreeC = chartsService.getDegreeC();
        twoCharts.setDegreeA(degreeA);
        twoCharts.setDegreeB(degreeB);
        twoCharts.setDegreeC(degreeC);
        String result = JSON.toJSONString(twoCharts);
        return result;
    }
    /*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<学生操作>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
    ...
    /*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<数据图表操作>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
    // 跳转页面
    @RequestMapping(value = "showCourseCharts")
    private String showCourseCharts(Model model) throws Exception {
        // 获取课程，学生，教师各个总数，并且设置到OneCharts对象中
        oneCharts = new OneCharts();
        int countCouse = courseService.getCountCouse();
        oneCharts.setCourseCount(countCouse);
        ...
        return "admin/showCourseCharts";
    }

    // 返回课程json
    @ResponseBody
    @RequestMapping(value = "showCourseChartsJson", method = RequestMethod.GET, produces = "application/json;charset=utf-8")
    private String showCourseChartsJson(Integer page) throws Exception {
        List<CourseCustom> list = null;
        //页码对象
        PagingVO pagingVO = new PagingVO();
        //设置总页数
...
//        List<Course> list = courseService.findAllCourse();
    ...
    @RequestMapping(value = "/showCeshi")
    private String showCeshi() {
        return "admin/ceshi";
    }
}
